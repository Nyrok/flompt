...
The official recommendation: 3–5 examples, wrapped in `<examples>` tags, covering edge cases and varied scenarios. Not just one example showing the ideal case, but examples that show Claude where the edges are.
...
This is exactly the ordering flompt enforces automatically. Not because it's arbitrary convention, but because it follows Anthropic's own recommendations for how Claude processes information most effectively.
